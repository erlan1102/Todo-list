    ...
    return (
        <div className='folder__title-block'>
            ...
            <span className='folder__title-change' onClick={()=> changeHandlerFolderTitle(item.id)}>
                {item.change
                ?  <span onClick={()=> setChange('')}><FontAwesomeIcon icon={faSyncAlt} /></span>
                :  <span onClick={()=> setChange(item.folder)}><FontAwesomeIcon icon={faPencilAlt} /></span>
                }
            </span>
